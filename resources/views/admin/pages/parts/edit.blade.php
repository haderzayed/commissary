            ...
            <div class="card-body">
                <div class="form-group">
                    ...
                    <input id="title" class="form-control"
                           name="title" placeholder="اسم الحزب "
                           required type="text" value="{{ $parts->title_id }}">
                ...
                <div class="form-group">
                    ...
                    <input id="image" class="form-control"
                           name="image"
                           required type="file">
                    <a href="{{ asset($parts->image) }}" target="_blank">
                        <img src="{{ asset($parts->image) }}" alt="{{ $hezps->title }}" title="{{ $hezps->title }}">
                    </a>
                </div>
            </div>
...
<script>
    ...
    jQuery('#edit_item_form').submit(function(e) {
        ...
        $.ajax({
            type:'POST',
            url: `{{ route('part.update', $hezps->id) }}`,
            data: formData,
            contentType: false,
            processData: false,
            success: (response) => {
